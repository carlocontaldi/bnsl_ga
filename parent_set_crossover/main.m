%% The Role of Crossover Operator in Bayesian Network Structure Learning Performance
% Ref: https://doi.org/10.1145/3071178.3071240
%% Init
...
algo = 'Std-GA_PSX';        % Standard GA
% algo = 'DiGSiR-GA_PSX';   % Diversity-Guided Site-specific Rate GA
...
alpha = 0.5;                % elite eligibility threshold (DiGSiR-GA)
d = struct('m0',1/5,'M0',3/5,'m1',1/10,'M1',1/2);	% healthy diversity interval (DiGSiR-GA)
...
for t = 1:T
    ...
    if strcmp(algo,'Std-GA_PSX')                    % S&S Phase
        [dag,eoer{t}(1,4),conv(t),eoer{t}(1,6)] = std_ga_psx(ss,data{t},N,M,MP,scoring_fn,bnet);   
    else
        [dag,eoer{t}(1,4),conv(t),eoer{t}(1,6)] = digsir_ga_psx(ss,data{t},N,M,MP,alpha,d,scoring_fn,bnet);
    end
